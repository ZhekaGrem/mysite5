import CssIcon from "./CssIcon";
import FigmaIcon from "./FigmaIcon";
import FirebaseIcon from "./FirebaseIcon";
import GitHubIcon from "./GitHubIcon";
import GitIcon from "./GitIcon";
import GoIcon from "./GoIcon";
import HtmlIcon from "./HtmlIcon";
import JestIcon from "./JestIcon";
import JQueryIcon from "./jQueryIcon";
import JsIcon from "./JsIcon";
import MongodbIcon from "./MongodbIcon";
import NextIcon from "./NextIcon";
import PhpIcon from "./PhpIcon";
import PostgreIcon from "./PostgreIcon";
import PythonIcon from "./PythonIcon";
import ReactIcon from "./ReactIcon";
import RedisIcon from "./RedisIcon";
import TailwindIcon from "./TailwindIcon";
import TsIcon from "./TsIcon";

export { CssIcon, FigmaIcon,FirebaseIcon,GitHubIcon, GitIcon, GoIcon, HtmlIcon, JestIcon, JQueryIcon, JsIcon, MongodbIcon, NextIcon, PhpIcon, PostgreIcon, PythonIcon,ReactIcon, TailwindIcon,TsIcon, RedisIcon}