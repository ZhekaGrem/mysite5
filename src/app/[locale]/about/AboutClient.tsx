...
const workValues = [
  {
    ...
    description:
      'I write self-documenting code with clear naming conventions and modular architecture. My codebase consistently scores over 90% on maintainability metrics, reducing technical debt by 35% on recent projects. I prioritize readability over cleverness and business value over perfectionism.',
  },
  {
    title: 'User Experience Engineer',
    description:
      'Every feature I build starts with user research and journey mapping. I`ve reduced bounce rates by 28% through performance optimizations and intuitive UI design. I regularly conduct usability testing sessions and iterate based on quantifiable user feedback rather than assumptions.',
  },
  {
    title: 'Performance Optimization Specialist',
    description:
      'I`ve cut average page load times from 4.2s to under 1.5s through strategic code splitting, asset optimization, and caching strategies. My applications maintain 60fps animations even on mid-tier mobile devices. I implement measurable performance budgets and monitor core web vitals as critical success metrics.',
  },
];
const hobbies = [
  {
...
const WorkValueCard = ({
  ...
  return (
    <motion.div
      ref={cardRef}
      style={{ x }}
      className={`relative grid grid-cols-12 gap-4 border-t-2 border-surface-light pt-8 dark:border-surface-dark ${index % 2 === 0 ? '' : 'text-right'}`}>
      {/* Number indicator */}
      <div
        className={`col-span-1 font-mono text-4xl font-bold text-gray-200 dark:text-gray-700 ${index % 2 === 0 ? 'text-left' : 'text-right'}`}>
        {String(index + 1).padStart(2, '0')}
      </div>

      {/* Content */}
      <div className={`col-span-11 ${index % 2 === 0 ? 'pl-8' : 'pr-8'}`}>
        <TypewriterText className="mb-4 text-2xl font-bold">{title}</TypewriterText>
        <div
          className={`relative ${index % 2 === 0 ? 'border-l-4' : 'border-r-4'} border-surface-light px-6 dark:border-surface-dark`}>
          <TypewriterText className="text-gray-600 dark:text-gray-400">{description}</TypewriterText>
        </div>
      </div>
    </motion.div>
  );
          ...
          <div className="space-y-6">
            {skills.languages.map((lang) => (
              <div key={lang.name} className="border-l-4 border-surface-light pl-4 dark:border-surface-dark">
                ...
                <TypewriterText className="text-gray-600 dark:text-gray-400">{lang.level}</TypewriterText>
              </div>
            ))}
          </div>
        </div>
              ...
              {index % 2 === 0 ? (
                <>
                  ...
                  <div className="col-span-5 pr-8">
                    <TypewriterText className="mb-2 text-2xl font-bold">{hobby.title}</TypewriterText>
                    <TypewriterText className="mb-4 text-gray-600 dark:text-gray-400">
                      {hobby.desc}
                    </TypewriterText>
                    <TypewriterText className="text-sm">{hobby.details}</TypewriterText>
                  </div>
      ...
      <Section className="my-16">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {[
            ...
            <motion.div key={advantage.title} className="bg-gray-100 p-6 dark:bg-gray-800" {...typewrite}>
              <H h="h3" className="mb-4 text-xl">
                {advantage.title}
              </H>
              <p>{advantage.desc}</p>
            </motion.div>
